...
def draw_example(ch, src_font, dst_font, canvas_size, x_offset, y_offset, filter_hashes): #这个函数返回用于生成一个含有两个字的512像素大的图片
    ...
    if dst_hash in filter_hashes: #如果没有这个匹配的字就不生成
        return None,None
    # print('qqq')
...
def font2img(src, dst, charset, char_size, canvas_size,
             x_offset, y_offset, sample_count, sample_dir, label=0, filter_by_hash=True): #sample_count 指定抽样数量
    src_font = ImageFont.truetype(src, size=char_size)
    dst_font = ImageFont.truetype(dst, size=char_size)

    filter_hashes = set()
    if filter_by_hash:
        filter_hashes = set(filter_recurring_hash(charset, dst_font, canvas_size, x_offset, y_offset)) #过滤哈希表的字符
    ...
    for c in charset:
        ...
        src,dst = draw_example(c, src_font, dst_font, canvas_size, x_offset, y_offset, filter_hashes)
        # e = draw_example(c, src_font, canvas_size, x_offset, y_offset, filter_hashes)
        if src:
            dst.save(os.path.join('trainB/', "%d_%04d.jpg" % (label, count)))
            src.save(os.path.join(sample_dir, "%d_%04d.jpg" % (label, count)))
            count += 1
            if count % 100 == 0:
...
parser.add_argument('--src_font', dest='src_font', required=True, default='/home/yxl/zi2zi/fonts/MSYHBD.TTF',help='path of the source font')
parser.add_argument('--dst_font', dest='dst_font', required=True, default='/home/yxl/zi2zi/fonts/train_fonts/ErZiYuanXiaoGaiDing-2.ttf',help='path of the target font')
...
parser.add_argument('--sample_count', dest='sample_count', type=int, default=1500, help='number of characters to draw')
